...
import numpy as np
import pickle
from cluster_SZ import cluster_info, power_spectrum, SF, healpix
import emcee
...
for amas in AMAS :
    ...
    x.append(torch.from_numpy(np.array(sf)))
    print('amas SF shape :',np.shape(sf))
...
def potential(theta):
    ...
    for posterior in posteriors:
        pot += posterior.potential(x=x[ind_amas],theta=theta)
        ind_amas += 1
    return pot
...
if noise:
    samp_path = 'fluc/join_chains_' + str(n_R500_low) + '-' + str(n_R500_high) + 'R500_noise.pkl'
else:
    samp_path = 'fluc/join_chains_'+str(n_R500_low)+'-'+str(n_R500_high)+'R500.pkl'
